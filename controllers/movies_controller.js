var MoviesController = {
  ...
  customersRentingThisFilm: function(req, res){
    var db = req.app.get('db')
    var title = req.params.title
    db.run('select customers.name, customers.phone, customers.account_credit, rentals.customer_id, rentals.returned_date, rentals.checkout_date from rentals, customers where rentals.title=$1 and rentals.returned_date is null;', [title], function(err, result){
      res.json(result)
    })
  },
